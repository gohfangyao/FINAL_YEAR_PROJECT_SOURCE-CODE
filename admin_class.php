...
Class Action {
	...
	function login(){
		extract($_POST);
		$type = array("","users","teacher_list","evaluator_list");
		$type2 = array("","admin","teacher","evaluator");
	...
	function login2(){
		extract($_POST);
			$qry = $this->db->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as name FROM evaluators where evaluator_code = '".$evaluator_code."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'password' && !is_numeric($key))
	...
	function save_teacher(){
		...
		$check = $this->db->query("SELECT * FROM teacher_list where email ='$email' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		$check = $this->db->query("SELECT * FROM teacher_list where school_id ='$school_id' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 3;
			exit;
		}
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO teacher_list set $data");
		}else{
			$save = $this->db->query("UPDATE teacher_list set $data where id = $id");
		}
	...
	function delete_teacher(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM teacher_list where id = ".$id);
		if($delete)
			return 1;
	}
	...
	function get_class(){
		extract($_POST);
		$data = array();
		$get = $this->db->query("SELECT c.id,concat(c.curriculum,' ',c.level,' - ',c.section) as class,s.id as sid,concat(s.code,' - ',s.subject) as subj FROM restriction_list r inner join class_list c on c.id = r.class_id inner join subject_list s on s.id = r.subject_id where r.teacher_id = {$fid} and academic_id = {$_SESSION['academic']['id']} ");
	...
	function get_report(){
		extract($_POST);
		$data = array();
		$get = $this->db->query("SELECT * FROM evaluation_answers where evaluation_id in (SELECT evaluation_id FROM evaluation_list where academic_id = {$_SESSION['academic']['id']} and teacher_id = $teacher_id and subject_id = $subject_id and class_id = $class_id ) ");
		$answered = $this->db->query("SELECT * FROM evaluation_list where academic_id = {$_SESSION['academic']['id']} and teacher_id = $teacher_id and subject_id = $subject_id and class_id = $class_id");
			$rate = array();
		while($row = $get->fetch_assoc()){

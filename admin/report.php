...
<?php $teacher_id = isset($_GET['fid']) ? $_GET['fid'] : '' ; ?>
<?php 
	...
	<div class="row">
		...
		<div class="col-md-9">
			...
			<table width="100%">
				...
				<p class=""><b>Total evaluator Evaluated: <span id="tse"></span></b></p>
			</div>
...
<script>
	...
	function load_class(){
		...
		$('#fname').text(fname[$('#teacher_id').val()])
		$.ajax({
			url:"ajax.php?action=get_class",
			method:'POST',
			data:{fid:$('#teacher_id').val()},
			error:function(err){
				console.log(err)
	...
	function load_report($teacher_id, $subject_id,$class_id){
		if($('#preloader2').length <= 0)
		start_load()
		$.ajax({
			url:'ajax.php?action=get_report',
			method:"POST",
			data:{teacher_id: $teacher_id,subject_id:$subject_id,class_id:$class_id},
			error:function(err){
				console.log(err)
				alert_toast("An Error Occured.","error");
				end_load()

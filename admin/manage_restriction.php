		...
		<div class="row">
			<div class="col-md-4 border-right">
				...
				<div class="form-group">
					...
					<select name="" id="teacher_id" class="form-control form-control-sm select2">
						<option value=""></option>
						<?php 
						$teacher = $conn->query("SELECT *,concat(firstname,' ',lastname) as name FROM teacher_list order by concat(firstname,' ',lastname) asc");
						$f_arr = array();
			...
			<div class="col-md-8">
				<table class="table table-condensed" id="r-list">
					...
					<tbody>
						...
						<tr>
							<td>
								<b><?php echo isset($f_arr[$row['teacher_id']]) ? $f_arr[$row['teacher_id']]['name'] : '' ?></b>
								<input type="hidden" name="rid[]" value="<?php echo $row['id'] ?>">
								<input type="hidden" name="faculty_id[]" value="<?php echo $row['teacher_id'] ?>">
							</td>
							<td>
...
<script>
	$(document).ready(function(){
		...
		$('#add_to_list').click(function(){
			...
			var fid = frm.find('#teacher_id').val()
			...
			var tr = $("<tr></tr>")
			tr.append('<td><b>'+f_arr[fid].name+'</b><input type="hidden" name="rid[]" value=""><input type="hidden" name="teacher_id[]" value="'+fid+'"></td>')
			...
			frm.find('#teacher_id').val('').trigger('change')
